...
제목: ⚠️ 아래 5가지 규칙을 하나라도 어기면 실패로 간주함. 반드시 모두 지킬 것.
[필수1] {title} 이름을 반드시 포함할 것
[필수2] 이 여행지만의 구체적 특징(볼거리·체험·계절·음식·지형 중 1~2가지)을 키워드로 반드시 포함할 것 — "여행", "명소", "추천"만 있으면 탈락
[필수3] 제목 마지막 단어는 반드시 명사로 끝낼 것 (예: ~코스, ~산책, ~야경, ~축제, ~해변, ~계곡, ~체험, ~한옥)
[필수4] 이모지 없이 30자 이내일 것
[필수5] "완벽 여행 가이드", "여행 정보", "여행지 소개", "총정리", "알아보기" 등 뻔하고 검색어 같은 표현은 절대 사용 금지
✅ 합격 예시: "내소사 전나무 숲길과 천년 고찰 산책 코스", "해운대 일출 명소와 마린시티 야경 코스", "제주 협재해변 에메랄드 바다와 한림공원 체험"
❌ 불합격 예시(이런 제목 쓰면 안 됨): "경복궁 완벽 여행 가이드", "부산 여행 명소 추천", "내소사(부안) 여행 정보 총정리"
